<main id="main" class="main-site">
        ...
        <div class=" main-content-area">
            ...
            <div class="col-md-12">
                <div class="wrap-address-billing">
                    <h3 class="box-title">{{ trans('checkout.Billing Address ') }} </h3>
                    <div class="billing-address">
                        <br/>

                        <p class="row-in-form">
                            <label for="fname">{{ trans('checkout.First Name') }}<span>*</span></label>
                            <input  type="text" name="fname" value="" placeholder="Your name" wire:model="firstname">
                            @error('firstname') <span class="text-danger">{{   $message }}</span> @enderror
                        </p>
                        <p class="row-in-form">
                            <label for="lname">{{ trans('checkout.Last Name') }}<span>*</span></label>
                            <input  type="text" name="lname" value="" placeholder="Your last name" wire:model="lastname">
                            @error('lastname') <span class="text-danger">{{   $message }}</span> @enderror

                        </p>
                        <p class="row-in-form">
                            <label for="email">{{ trans('checkout.Email Address') }}:</label>
                            <input  type="email" name="email" value="" placeholder="Type your email"wire:model="email">
                            @error('email') <span class="text-danger">{{   $message }}</span> @enderror

                        </p>
                        <p class="row-in-form">
                            <label for="phone">{{ trans('checkout.Phone Number') }}<span>*</span></label>
                            <input  type="number" name="phone" value="" placeholder="10 digits format" wire:model="mobile">
                            @error('mobile') <span class="text-danger">{{   $message }}</span> @enderror

                        </p>
                        <p class="row-in-form">
                            <label for="add"> {{ trans('checkout.line1') }}:</label>
                            <input  type="text" name="add" value="" placeholder="Street at apartment number" wire:model="line1">
                            @error('line1') <span class="text-danger">{{   $message }}</span> @enderror

                        </p>
                        <p class="row-in-form">
                            <label for="add">{{ trans('checkout.line2') }}:</label>
                        ...
                        <p class="row-in-form">
                            <label for="country">{{ trans('checkout.Country') }}<span>*</span></label>
                        ...
                        <p class="row-in-form">
                            <label for="province"> {{ trans('checkout.Province') }}</span></label>
                            <input  type="text" name="province" value="" placeholder="province" wire:model="province">
                            @error('province') <span class="text-danger">{{   $message }}</span> @enderror

                        </p>
                        <p class="row-in-form">
                            <label for="city">{{ trans('checkout.City') }}<span>*</span></label>
                            <input  type="text" name="city" value="" placeholder="City name" wire:model="city">
                            @error('city') <span class="text-danger">{{   $message }}</span> @enderror

                        </p>
                        <p class="row-in-form">
                            <label for="zip-code">{{ trans('checkout.Postcode') }}:</label>
                        ...
                        <p class="row-in-form fill-wife">
                            ...
                            <label class="checkbox-field">
                                ...
                                <span> {{ trans('checkout.Ship to a different address?') }}</span>
                            </label>
                        </p>
                        <br/>
            ...
            <div class="col-md-12">
                <div class="wrap-address-billing">
                    <h3 class="box-title"> {{ trans('checkout.shiping Address') }}</h3>
                    <div class="billing-address">
                        <br/>
            ...
            <div class="summary summary-checkout">
                <div class="summary-item payment-method">
                    <h4 class="title-box">{{ trans('checkout.Payment Method ') }}</h4>
                    <p class="summary-info"><span class="title">{{ trans('checkout.Money order') }}</span></p>
	...
	<div wire:ignore id="processing" style="font-size:22px; margin-bottom:20px;padding-left:37px;color:green;display:none;">
		...
		<span> {{ trans('checkout.Processing') }}...</span>
	</div>
@endif
                    <button type="submit" class="btn btn-medium"> {{ trans('checkout.order now ') }}</button>
                </div>
                <div class="summary-item shipping-method">
                    <h4 class="title-box f-title">{{ trans('checkout.Shipping method ') }}</h4>
                    <p class="summary-info"><span class="title">{{ trans('checkout.Flat Rate') }}</span></p>
                    <p class="summary-info"><span class="title">{{ trans('checkout.Fixed') }} $50.00</span></p>
